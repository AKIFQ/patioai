-- Fix migration function to work without ID column
...
DECLARE
  ...
  room_password text;
  anonymous_participant_session text;
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    IF NOT existing_participant THEN
      ...
      SELECT rp.session_id, rp.display_name, rp.joined_at 
      INTO anonymous_participant_session, anonymous_participant_name, anonymous_participant_joined
      ...
      IF FOUND THEN
        ...
        RAISE NOTICE 'Identity migration detected: anonymous participant % (session: %) will be migrated to user_id %', 
          anonymous_participant_name, anonymous_participant_session, p_user_id;
      END IF;
    END IF;
  ELSE
  ...
  ELSIF migration_needed THEN
    ...
    -- Step 1: Update the participant record (using session_id as identifier)
    UPDATE room_participants 
    ...
    WHERE room_id = p_room_id 
      AND session_id = anonymous_participant_session
      AND user_id IS NULL;
