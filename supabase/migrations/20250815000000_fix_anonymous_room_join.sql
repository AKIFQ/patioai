-- Fix anonymous user room joining by updating join_room_safely function
-- The issue was that the function only checked user_id for existing participants,
-- but anonymous users have user_id = null, so it failed to detect existing participants
