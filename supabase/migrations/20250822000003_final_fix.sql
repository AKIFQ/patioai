-- Final fix for identity migration function
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    IF NOT existing_participant THEN
      ...
      SELECT p.id, p.display_name, p.joined_at 
      INTO anonymous_participant_id, anonymous_participant_name, anonymous_participant_joined
      FROM room_participants p
      WHERE p.room_id = p_room_id 
        AND p.user_id IS NULL  -- Anonymous participant
        AND (
          -- Match by previous session ID if provided
          (p_previous_session_id IS NOT NULL AND p.session_id = p_previous_session_id)
          OR 
          -- Match by session pattern (e.g., session_123 → auth_user-id)
          (p_previous_session_id IS NULL AND p.session_id LIKE 'session_%')
          OR
          -- Match by display name as fallback (less reliable but better than nothing)
          (p_previous_session_id IS NULL AND p.display_name = p_display_name)
        )
      LIMIT 1;
