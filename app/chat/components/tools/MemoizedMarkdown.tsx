    ...
    return (
      <ReactMarkdown
        components={{
          ...
          p: ({ children }) => (
            <p className={`${compact ? 'mb-1 last:mb-0 text-xs leading-tight' : 'mb-4 last:mb-0'} break-words overflow-wrap-anywhere`}>{children}</p>
          ),
          ...
          // Lists
          ul: ({ children }) => (
            <ul className={`list-disc ${compact ? 'pl-3 mb-1 space-y-0' : 'pl-6 mb-4 space-y-1'}`}>{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className={`list-decimal ${compact ? 'pl-3 mb-1 space-y-0' : 'pl-6 mb-4 space-y-1'}`}>{children}</ol>
          ),
          li: ({ children }) => <li className={`${compact ? 'mb-0 text-xs' : 'mb-1'} break-words`}>{children}</li>,
          // Headings
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold mb-4 mt-6">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className={compact ? 'text-sm font-semibold mb-1 mt-2 break-words' : 'text-xl font-bold mb-3 mt-5'}>{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className={compact ? 'text-xs font-semibold mb-1 mt-2 break-words' : 'text-lg font-bold mb-2 mt-4'}>{children}</h3>
          ),
          h4: ({ children }) => (
            <h4 className={compact ? 'text-[11px] font-semibold mb-0.5 mt-1.5 break-words' : 'text-base font-bold mb-2 mt-4'}>{children}</h4>
          ),
          h5: ({ children }) => (
            <h5 className={compact ? 'text-[10px] font-semibold mb-0.5 mt-1 break-words' : 'text-sm font-bold mb-2 mt-3'}>{children}</h5>
          ),
          h6: ({ children }) => (
            <h6 className={compact ? 'text-[9px] font-semibold mb-0.5 mt-1 break-words' : 'text-xs font-bold mb-2 mt-3'}>{children}</h6>
          ),
          // Block elements
          blockquote: ({ children }) => (
          ...
          code({ className, children, ...props }) {
            ...
            if (inline) {
              return (
                <code
                  className={`bg-muted px-1 py-0.5 rounded ${compact ? 'text-[10px]' : 'text-sm'} break-words overflow-wrap-anywhere ${className}`}
                  {...props}
                >
                  {children}
                </code>
            ...
            return (
              <div className={`relative rounded w-full ${compact ? 'pt-3 my-1' : 'pt-5 my-2'} max-w-full`}>
                ...
                <pre className={`m-0 overflow-x-auto bg-muted ${compact ? 'p-2' : 'p-3 sm:p-4'} rounded-md ${compact ? 'text-[10px]' : 'text-sm'} max-w-full`}>
